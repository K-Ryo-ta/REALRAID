...
const CreateRoomPass = () => {
  ...
  const checkAnswer = () => {
	...
	const handleSubmit = () => {
		const tmpAnswer = answerItems.reduce(
			(arr, item) => arr + item.content,
			""
		);
	...
	const handleDragEnd = (event: { active: any; over: any }) => {
		...
		if (
			...
			const oldIndex = answerItems.findIndex(
				(item) => item.id === active.id
			);
			const newIndex = answerItems.findIndex(
